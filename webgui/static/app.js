(function () {
  ...
  const artifactIcon = kind => kind === 'image' || kind === 'video' ? 'result' : kind === 'json' ? 'file' : 'layers';
  ...
  async function previewArtifact(path, kind) {
    ...
    try {
      if (kind === 'image' || /\.(png|jpg|jpeg|gif|pgm)$/i.test(path)) {
        ...
        artifactPreview.innerHTML = `<video src="${url}" controls autoplay playsinline preload="metadata" aria-label="${escapeHtml(path)}"></video>`;
      } else {
