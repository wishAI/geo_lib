...
export function mount(root) {
  ...
  root.innerHTML=`<div class="char-editor">
    ...
    <div class="char-layout"><section class="char-viewport"><canvas aria-label="Landau 3D editor — drag to orbit, right drag to pan, scroll to zoom"></canvas>
      ...
      <div class="char-pane" data-pane="shape" hidden><p class="char-hint">Rest-shape controls move the actual mesh. Test expressions again after changing proportions.</p><div data-shape-sliders></div></div>
      <div class="char-pane" data-pane="parts" hidden><p class="char-hint">Garments are independent objects. The skin underneath is a reconstructed approximation.</p><div class="char-presets"><button data-action="undress">Hide clothing</button><button data-action="dress">Show clothing</button></div><div data-part-list></div><div class="char-section"><h3 data-part-title>Select a part</h3><div class="char-presets"><button data-action="isolate">Isolate selected</button><button data-action="show-character">Show character</button></div><label class="char-bone-select">Material <select aria-label="Selected material" data-material></select></label><label>Material color <input aria-label="Selected material color" type="color" value="#ffffff" data-tint></label><button data-action="untint">Reset material color</button></div></div>
  ...
  function validatePreset(p){
    ...
    for(const [n,v] of Object.entries(p.morphs||{})){if(!morphs.has(n)||!Number.isFinite(v)||v<(SHAPES.has(n)?-1:0)||v>1)throw new Error('Invalid morph: '+n);}
  ...
  function expression(kind){query('[data-blink]').checked=false;for(const n of morphs.keys())if(!SHAPES.has(n))setMorph(n,0,false);const p={neutral:{},happy:{mouthSmile:.7,eyeSquintL:.15,eyeSquintR:.15},surprised:{jawDrop:.75,browUpL:.65,browUpR:.65,eyeWideL:.5,eyeWideR:.5},half:{eyeBlinkL:.5,eyeBlinkR:.5},blink:{eyeBlinkL:1,eyeBlinkR:1},look:{eyeLookOutL:.75,eyeLookInR:.75}}[kind];for(const [n,v]of Object.entries(p))setMorph(n,v,false);touch();announce('Expression: '+kind);}
  function control(name){const min=SHAPES.has(name)?-1:0;return `<label class="char-slider"><span>${esc(pretty(name))}</span><div><input aria-label="${esc(pretty(name))}" data-morph="${esc(name)}" type="range" min="${min}" max="1" step=".01" value="0"><output>0.00</output></div></label>`;}
  ...
  root.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='undress'||action==='dress'){for(const [n,p]of parts)if(p.kind==='clothing')settings.parts[n]={...settings.parts[n],visible:action==='dress'};for(const [n,p]of parts)if(p.kind==='inferred_body')settings.parts[n]={...settings.parts[n],visible:action==='undress'};applyParts();shading();root.querySelectorAll('[data-visible]').forEach(el=>el.checked=settings.parts[el.dataset.visible]?.visible??parts.get(el.dataset.visible).defaultVisible);touch();}
  ...
  root.addEventListener('change',async e=>{
    ...
    if(el.hasAttribute('data-reference')){const url=asset('inputs/landau_v10/'+el.value);query('[data-reference-image]').src=url;query('[data-reference-link]').href=url;query('[data-reference-frame]').dataset.source=el.value;}
  ...
  const ready=(async()=>{try{
    ...
    model.traverse(o=>{
      ...
      if(o.userData.part_type)parts.set(o.name,{object:o,kind:o.userData.part_type,defaultVisible:!o.userData.default_hidden&&o.userData.part_type!=='inferred_body',materials:new Set()});
    });
    ...
    query('[data-face-sliders]').innerHTML=[...morphs.keys()].filter(n=>!SHAPES.has(n)).sort().map(control).join('');
    ...
    query('[data-loading]').hidden=true;selectPart('Head');readBone();shading();view('face');announce('Ready. Edits are saved in this browser; download a preset for backup.');
